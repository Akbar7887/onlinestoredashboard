...
class ProductPage extends GetView<CatalogController> {
    ...
    if (catalogs.length > 0) {
      catalogs.forEach((catalog) {
        ...
        catalog.products!.forEach((product) {
          _controller.products.add(product);
        });
                  ...
                  onCellTap: (cell) async {
                    if (cell.rowColumnIndex.rowIndex > -1) {
                      if (cell.rowColumnIndex.columnIndex == 2) {
                        _product =
                        _controller.products[cell.rowColumnIndex.rowIndex - 1];
  ...
  List<TreeNode> treeList(List<Catalog> list) {
    return list
        ...
        content: InkWell(
            onTap: () {
              _controller.products = _controller.products
                  .where((element) => element.catalogId == e.id)
                  .toList();
              _productDataGridSource = ProductDataGridSource(_controller.products);
            },
            child: Container(
                height: 50,
                width: 200,
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      return _controller.products.length == 0
          ? Center(
